...
describe("createApiClient", () => {
  ...
  it("passes through a 204 response without attempting envelope parsing", async () => {
    const client = createApiClient({
      runtimeConfig,
      tokenProvider: {
        getAccessToken: () => "access-token",
      ...
      adapter: createAdapter((config) =>
        createResponse(config, undefined, 204),
      ),
    });

    const response = await client.instance.delete("/api/widgets/widget-1");
