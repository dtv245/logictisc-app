...
export function resolveHealthProbeState(
  ...
  // Chuyển transport result sang discriminated union dùng trực tiếp bởi UI;
  // nhờ đó component không phải suy luận trạng thái từ nhiều boolean rời rạc.
  switch (result.kind) {
    case "cors-blocked":
      return {
        config,
    ...
    case "healthy": {
      ...
      // API UP chưa đủ để mở route nghiệp vụ: database phải được backend xác
      // nhận rõ là enabled. Giá trị lạ được xử lý fail-closed như unhealthy.
