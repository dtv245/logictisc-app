...
import type { VehicleMapPoint } from "../operations.data";
...
export const VehicleTrackingMap = ({
  ...
  // Effect hợp lệ theo rule 6.1: điều khiển SDK bên thứ ba (Leaflet) trên một DOM
  // node, việc này không query library nào quản lý được. Leaflet nạp động để bản đồ
  // không nằm trong bundle chính.
  //
  // Cleanup bắt buộc: StrictMode mount hai lần, không `remove()` map cũ sẽ để lại
  // hai instance Leaflet cùng trỏ vào một container.
  useEffect(() => {
    ...
    return (
      <Empty
        ...
        image={inaccessibleText
          ? <LockOutlined className="operations-empty-icon" />
          : <EnvironmentOutlined className="operations-empty-icon" />}
      />
    );
  }
