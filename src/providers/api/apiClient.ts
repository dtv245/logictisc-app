...
export const createApiClient = (
  ...
  instance.interceptors.response.use(
    (response: AxiosResponse<unknown>): AxiosResponse<unknown> => {
      ...
      // A successful 204 intentionally has no JSON body. Passing it through
      // lets deleteOne build Refine's required `{ id }` result without
      // weakening envelope validation for any response that contains data.
